...
module.exports.updateBookByTitle = function(db, redis, title, text, callback) {
	console.log(title);
	db.collection("text").update({title:title}, {$set:{text:text}}, function (err, doc) { //Update the main database
		if(err)
			callback(err);
		else if (!doc)
